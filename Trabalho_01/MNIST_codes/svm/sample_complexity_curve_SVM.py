...
for i in range(20):
    ...
    svm_sem_ruido = make_pipeline(LinearSVC(random_state=42, dual='auto', C=0.17787658410143284, class_weight='balanced'))
    svm_sem_ruido.fit(X,y)
    
    # Test sem ruido
    svm_sem_ruido_predicts = svm_sem_ruido.predict(Xt)
    n_erros = np.sum(svm_sem_ruido_predicts != test_labels)
    taxa_erro_svm_sem_ruido = 100 - ((n_erros/len(test_labels)) * 100)

    print(f"SVM com ", porcentagem ,"% do conjunto de treino")
    print(f"Predicts: {svm_sem_ruido_predicts}")
    print("Valores Esperados:", test_labels)
    print("Taxa de acerto:", taxa_erro_svm_sem_ruido, "%")
    sampleValues.append((porcentagem/100) * 56000)
    accuracyValuesArray.append(taxa_erro_svm_sem_ruido) 
