...
for i in range(20):
    ...
    taxa_erro_tree = 100 - ((n_erros/len(test_labels)) * 100)
    
    print(f"DTree com ", porcentagem ,"%")
    print(f"Predicts: {tree_predicts}")
    print("Valores Esperados:", test_labels)
    print("Taxa de acerto:", taxa_erro_tree, "%")
    sampleValues.append((porcentagem/100) * 400)
    accuracyValuesArray.append(taxa_erro_tree) 
