...
for i in range(20):
    ...
    knn_sem_ruido.fit(X,y)
    
    # Test sem ruido
    knn_sem_ruido_predicts = knn_sem_ruido.predict(Xt)
    n_erros = np.sum(knn_sem_ruido_predicts != test_labels)
    taxa_erro_knn_sem_ruido = 100 - ((n_erros/len(test_labels)) * 100)

    print(f"KNN com ", porcentagem ,"% do conjunto de treino, K=15")
    print(f"Predicts: {knn_sem_ruido_predicts}")
    print("Valores Esperados:", test_labels)
    print("Taxa de acerto:", taxa_erro_knn_sem_ruido, "%\n")
    sampleValues.append((porcentagem/100) * 56000)
    accuracyValuesArray.append(taxa_erro_knn_sem_ruido) 
    porcentagem += 5
